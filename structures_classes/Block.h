...
public:
    ...
    Block(uint64_t idx, uint64_t ts, std::vector<uint8_t> prevHash, std::vector<Transaction> txs, std::vector<uint8_t> hash)
        : index(idx), timestamp(ts), previousBlockHash(prevHash), transactions(txs), blockHash(hash) {}
    ...
    static Block newBlock(uint64_t index, uint64_t timestamp, std::vector<uint8_t> previousBlockHash, std::vector<Transaction> transactions);
};
